...
[Default of Credit Card Clients](https://archive.ics.uci.edu/dataset/350/default+of+credit+card+clients), UCI Machine Learning Repository. 30,000 credit card clients in Taiwan, April to September 2005. Amounts are in New Taiwan dollars.
...
- **Results are sensitive to library versions, more than expected.** Re-running `05_current_subgroup.ipynb` in a different environment (same code, same `random_state`) produced a materially different result for the subgroup ablation — see Section 6. `requirements.txt` now pins `scikit-learn` and `xgboost`, so re-running against it going forward should stop this drift, but the two conflicting numbers in Section 6 predate the pin and haven't been reconciled by re-running under it.
...
1. The raw dataset is already included in the repo at `data/raw/UCI_Credit_Card.csv` (also zipped at the repo root as `UCI_Credit_Card.csv.zip`) — no download needed.
2. Install dependencies: `pip install -r requirements.txt`.
3. Run the notebooks in order, `01` through `06`. Each downstream notebook depends on a CSV produced by an earlier one (`credit_card_cleaned.csv` from 01, `credit_card_featured.csv` from 03), so run them top to bottom in sequence rather than out of order.